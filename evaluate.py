...
## Network Arguments
parser = argparse.ArgumentParser(description='Evaluation: social-informed Trajectory Prediction for Autonomous Driving')
...
parser.add_argument('--metric',   type=str, help='RMSE & NLL is calculated by (agent/sample) based evaluation', default="sample")
...
def maskedMSETestMode(y_pred, y_gt, mask, separately=False):
...
def model_evaluate():
    ...
    ## Initialize network
    TraJ= TrajPred(args) 
    TraJ.load_state_dict(torch.load('./trained_models/{}/{}.tar'.format((args.name).split('-')[0], args.name)))
    if args.use_cuda:
        TraJ=TraJ.cuda() 
    ...
    # count shape of data #################
	#man_lat_LK  = torch.zeros([1, 3]).cuda()
    #man_lat_LCL = torch.zeros([1, 3]).cuda()
    #man_lat_LCR = torch.zeros([1, 3]).cuda()

    #man_lon_acc= torch.zeros([1, 2]).cuda()
    #man_lon_brk= torch.zeros([1, 2]).cuda()
    #######################################
    ref = torch.tensor([0., 1., 0.])
    ...
    with torch.no_grad():
        for i, data in enumerate(tsDataloader):
            ...
            #print('lat_enc:', lat_enc[0])
            #for kk in range(0, lat_enc.shape[0]):
            #	print('kk:', kk)
           # 	lat_enc_element=lat_enc[kk] 
           # 	print('one element:', lat_enc_element)
            	#if lat_enc_element == ref:
            	#	print(torch.stack(lat_enc_element, dim=0))

            	#else:
            	#	continue

            	#print('# elements:', lat_enc_element)
            #print('lat_enc_shape:', lat_enc.shape[0])

            #print('lon_enc_shape:', lon_enc.shape[0])
            ...
            #print('LatMan.shape: ', lat_enc.shape)
            #print('LonMan.shape:' , lon_enc.shape)
            #print('targsEncMask.shape:' , targsEncMask.shape)
            ...
            if args.metric == 'agent':
                ...
                l, c = maskedNLLTest(fut_pred, lat_pred, lon_pred, targsFut, targsFutMask, separately=False)
                ...
                ###llmodeX, ccmodeY = maskedMSETestMode(fut_pred_max, targsFut, targsFutMask, separately=False)	
                ...
                #llmodeX = llmodeX.detach().cpu().numpy()
                #llmodeY = ccmodeY.detach().cpu().numpy() 

                #print('llmodeX: ', llmodeX) 
                #print('llmodeY: ', llmodeX)	
