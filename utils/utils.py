...
def maskedNLLTest(fut_pred, lat_pred, lon_pred, fut, op_mask,
    ...
    if use_maneuvers:
        ...
        # Check and reshape fut_pred if necessary
        print("fut_pred shape before loop:", fut_pred.shape)

        # If fut_pred has fewer dimensions, reshape it accordingly
        if fut_pred.ndim == 2:
            fut_pred = fut_pred.unsqueeze(1)  # Add dimension if needed

        # Make sure fut_pred has the expected dimensions
        assert fut_pred.ndim == 3, "fut_pred should be a 3D tensor after reshaping"
        ...
        for k in range(num_lon_classes):
            for l in range(num_lat_classes):
                ...
                # Ensure y_pred is correctly indexed
